...
q11_1 = pd.pivot_table(data=dataset11, index='Country', columns='year',
                       values='Happiness_Score')

q11_1['na_num'] = q11_1.isna().sum(axis=1) # axis = 1 행별로 결측치 수 셀수 있음

q11_1_2 = q11_1[q11_1['na_num'] < 1]

ans1 = (q11_1['na_num'] > 0).sum()
...
q11_2 = q11_1_2.drop(columns='na_num')
...
# 변수 2개 ttest, 3개 이상 ANOVA 
# 귀무가설 : 평균이 같다
# 대립가설 : 적어도 하나는 평균이 다르다. 
# ANOVA : 분산분석 -> F 분포: 집단간의 흩어진 정도, 자유도 n-1

from statsmodels.formula.api import ols
from statsmodels.stats.anova import anova_lm

# C(수치형 변수) : 범주형으로 인식
q11_3 = dataset11[dataset11['Country'].isin(q11_2.index.values)]

anova = ols('Happiness_Score~C(year)', q11_3).fit() 

# ols 사용 시 변수명에서 스페이스가 포함된 변수, -가 포함된 변수, . 포함된 변수 사용 못함
...
# PR 이 p-value
# PR > 0.05 이기때문에 귀무가설을 기각하지 않음
# df 자유도

# (정답) 0.0042

# 어떤집단끼리 차이나는지 알 수 있는 함수(?)
from statsmodels.stats.multicomp import pairwise_tukeyhsd

q11_3 = pairwise_tukeyhsd(q11_3['Happiness_Score'], q11_3['year'])

print(q11_3)

from scipy.stats import f_oneway

f_oneway(q11_1_2[2015].values, q11_1_2[2016].values, q11_1_2[2017].values)
...
# q13_1 = dataset13[['experience', 'last_new_job']]['gender'].corr()
...
dr_q2 = \
df.loc[(df['major_displine'] == 'STEM') &
       df['city_development_index'] > df['city_development_index'].quantile(0.85), ]

chi_contingency(pd.crosstab(df_q2['relevent_experience'], df_q2['target']))
